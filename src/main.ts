...
export const loop = ErrorMapper.wrapLoop(() => {
    ...
    if (Game.spawns["Spawn1"].room.energyCapacityAvailable == 550) {
        ...
        if (Object.values(Game.creeps).length < 8) {
            Game.spawns["Spawn1"].spawnCreep([MOVE, MOVE, MOVE, CARRY, CARRY, CARRY, CARRY, WORK, WORK], Game.time.toString(), {
                memory: {
                    status: 0,
    ...
    } else if (Object.values(Game.creeps).length < 8) {
